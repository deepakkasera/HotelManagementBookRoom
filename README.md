...
1. A customer can book multiple rooms before requesting for the bill. Hence, our system must be able to track all the rooms booked by a customer.
...
2. The request for booking a room will contain the user id of the customer and a Map<Long,Integer>, where the key will be rooms id and value is the quantity of rooms.
...
5. If a booking contains a rooms which is not present in the database, then we should throw an error.
